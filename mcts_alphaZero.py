...
class TreeNode(object):
    ...
    def __init__(self, parent, prior_p):
        ...
        self._u = 0
        self._P = (
            prior_p  # its the prior probability that action's taken to get this node
        )
    ...
    def get_action(self, board, is_selfplay, print_probs_value):
        ...
        if len(sensible_moves) > 0:
            if is_selfplay:
                ...
                if (
                    board.width * board.height - len(board.availables)
                    <= self.first_n_moves
                ):
                    # For the first n moves of each game, the temperature is set to τ = 1
                    temp = 1
            ...
            if print_probs_value and move_probs is not None:
                act_probs, value = self.policy_value_function(
                    board, self.action_fc, self.evaluation_fc
                )
                print("-" * 10)
                print("value", value)
                ...
                probs = (
                    np.array(move_probs)
                    .reshape((board.width, board.height))
                    .round(3)[::-1, :]
                )
                for p in probs:
                    for x in p:
