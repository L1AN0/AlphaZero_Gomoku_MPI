    ...
    def current_state(self):
        ...
        if self.states:
            ...
            for i in range(self.feature_planes):
                ...
                if i % 2 == 0:
                    square_state[i][
                        move_oppo // self.width, move_oppo % self.height
                    ] = 1.0
                else:
                    square_state[i][
                        move_curr // self.width, move_curr % self.height
                    ] = 1.0
            ...
            for i in range(0, len(self.states_sequence) - 2, 2):
                for j in range(i + 2, len(self.states_sequence), 2):
                    if self.states_sequence[i][1] != -1:
                        assert (
                            square_state[j][
                                self.states_sequence[i][0] // self.width,
                                self.states_sequence[i][0] % self.height,
                            ]
                            == 1.0
                        ), "wrong oppo number"
                        square_state[j][
...
class Game(object):
    ...
    def start_play(self, player1, player2, start_player=0, is_shown=1, print_prob=True):
        ...
        while True:
            ...
            move, move_probs = player_in_turn.get_action(
                self.board, is_selfplay=False, print_probs_value=print_prob
            )
    ...
    def start_play_with_UI(self, AI, start_player=0):
        ...
        while True:
            ...
            if current_player == 1 and not end:
                move, move_probs = AI.get_action(
                    self.board, is_selfplay=False, print_probs_value=1
                )
            else:
                inp = UI.get_input()
    ...
    def start_self_play(self, player, is_shown=0):
        ...
        while True:
            move, move_probs = player.get_action(
                self.board, is_selfplay=True, print_probs_value=False
            )
            # store the data
