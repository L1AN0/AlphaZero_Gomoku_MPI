...
class PolicyValueNet:
    def __init__(
        ...
        for params in self.network_params:
            # print(params,'**'*100)
            if (
                ("conv2d" in params.name)
                or ("resnet" in params.name)
                or ("bn" in params.name)
                or ("flatten_layer" in params.name)
            ):
        ...
        self.action_fc_train_oppo, self.evaluation_fc2_train_oppo = self.network(
            input_states=self.input_states, reuse=False, is_train=True, label="_oppo"
        )
        self.action_fc_test_oppo, self.evaluation_fc2_test_oppo = self.network(
            input_states=self.input_states, reuse=True, is_train=False, label="_oppo"
        )
    ...
    def policy_value(self, state_batch, actin_fc, evaluation_fc):
        ...
        log_act_probs, value = self.session.run(
            [actin_fc, evaluation_fc], feed_dict={self.input_states: state_batch}
        )
    ...
    def policy_value_fn_random(self, board, actin_fc, evaluation_fc):
        ...
        equi_mcts_prob = np.flipud(
            act_probs[0].reshape(self.board_height, self.board_width)
        )
        if flip:
    ...
    def network(self, input_states, reuse, is_train, label=""):
        ...
        with tf.variable_scope("model" + label, reuse=reuse):
            ...
            residual_layer = self.residual_block(
                incoming=conv1,
                out_channels=64,
                is_train=is_train,
            ...
            action_conv = tl.layers.BatchNormLayer(
                action_conv, act=tf.nn.relu, is_train=is_train, name="bn_1"
            )
            action_conv_flat = tl.layers.FlattenLayer(
                action_conv, name="flatten_layer_1"
            )
            # 3-2 Full connected layer,
            # the output is the log probability of moves on each slot on the board
            action_fc = tl.layers.DenseLayer(
                action_conv_flat,
                n_units=self.board_width * self.board_height,
                act=tf.nn.log_softmax,
                name="dense_layer_1",
            )
            # 4 Evaluation Networks
            # these layers are the same as paper's
            evaluation_conv = tl.layers.Conv2d(
                residual_layer,
                n_filter=1,
                filter_size=(1, 1),
                strides=(1, 1),
            ...
            evaluation_conv = tl.layers.BatchNormLayer(
                evaluation_conv, act=tf.nn.relu, is_train=is_train, name="bn_2"
            )
            evaluation_conv_flat = tl.layers.FlattenLayer(
                evaluation_conv, name="flatten_layer_2"
            )
            evaluation_fc1 = tl.layers.DenseLayer(
                evaluation_conv_flat, n_units=256, act=tf.nn.relu, name="dense_layer_2"
            )
            evaluation_fc2 = tl.layers.DenseLayer(
                evaluation_fc1, n_units=1, act=tf.nn.tanh, name="flatten_layer_3"
            )
    ...
    def residual_block(self, incoming, out_channels, is_train, nb_block=1):
        ...
        for i in range(nb_block):
            ...
            resnet = tl.layers.Conv2d(
                resnet,
                n_filter=out_channels,
                filter_size=(3, 3),
                strides=(1, 1),
                padding="SAME",
                name="resnet_conv2d_" + str(i) + "_1",
            )
            resnet = tl.layers.BatchNormLayer(
                resnet,
                is_train=is_train,
                act=tf.nn.relu,
            ...
            resnet = tl.layers.Conv2d(
                ...
                strides=(1, 1),
                padding="SAME",
                name="resnet_conv2d_" + str(i) + "_2",
            )
            resnet = tl.layers.BatchNormLayer(
                resnet, is_train=is_train, name="resnet_bn_" + str(i) + "_2"
            )

            resnet = tl.layers.ElementwiseLayer(
                [resnet, identity],
                combine_fn=tf.add,
                name="elementwise_layer_" + str(i),
            )
            resnet = MyActLayer(
                resnet, act=tf.nn.relu, name="activation_layer_" + str(i)
            )
...
class MyActLayer(Layer):
    ...
    def __init__(
        self, prev_layer=None, act=tf.identity, name="activation_layer",
    ):
