...
class TrainPipeline:
    def __init__(self, init_model=None, transfer_model=None):
        ...
        if (init_model is not None) and os.path.exists(init_model + ".index"):
            ...
            self.policy_value_net = PolicyValueNet(
                self.board_width, self.board_height, block=self.resnet_block, cuda=True
            )
    ...
    def collect_selfplay_data(self, n_games=1):
        ...
        for i in range(n_games):
            winner, play_data = self.game.start_self_play(
                self.mcts_player, is_shown=False
            )
        ...
        try:
            for i in range(self.game_batch_num):
                ...
                if len(self.data_buffer) > self.batch_size * 5:
                    ...
                    print(
                        "collect_data_time : {}, train_data_time : {},evaluate_time : {}".format(
                            collect_data_time / 3600,
                            train_data_time / 3600,
                            evaluate_time / 3600,
                        )
                    )
...
if __name__ == "__main__":
    training_pipeline = TrainPipeline(
        init_model="model/best_policy.model", transfer_model=None
    )
